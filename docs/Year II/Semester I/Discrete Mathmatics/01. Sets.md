...
$$\begin{aligned}\text{Claim: }&\pset{A}\sseq\pset B\text{, Given that }A\sseq B\\\\
\text{Proof: }&\text{Suppose }x\in\pset A\\
\Rightarrow&\ x\sseq A \sseq B\\
\Rightarrow&\ x \sseq B\\
\Rightarrow&\ x \in\pset B\\\\
\Rightarrow&\ \forall x \in\pset A,\ \exists x\in\pset B\\
\Rightarrow&\ \pset A \sseq\pset B\\\\
\end{aligned}$$